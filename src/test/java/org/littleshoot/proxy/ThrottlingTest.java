...
public class ThrottlingTest {
  ...
  public void testThrottledWrite() throws Exception {
    ...
    assertThat(
        "Expected throttled write to complete in approximately " + msToWriteThrottled + "ms" + " but took " + (finish
                                                                                                               - start)
        + "ms",
  ...
  public void testUnthrottledWrite() throws Exception {
    ...
    assertThat(
        "Unthrottled write took " + (finish - start) + "ms, but expected to complete in " + UNTRHOTTLED_REQUEST_TIME_MS
        + "ms",
  ...
  @Ignore("This starts failing with netty 4.1.53")
  public void testThrottledRead() throws Exception {
    ...
    log.info("To read: " + readContent.length);
    int bytesRead = 0;
    while (bytesRead < largeData.length) {
      int read = response.getEntity().getContent()
    ...
    final double elapsed = finish - start;
    assertThat(
        "Expected throttled read to complete in approximately " + msToReadThrottled + "ms" + " but took " + elapsed
        + "ms",
        elapsed, both(greaterThan(msToReadThrottled * (1 - ALLOWABLE_VARIATION)))
  ...
  public void testUnthrottledRead() throws Exception {
    ...
    assertThat(
        "Unthrottled read took " + (finish - start) + "ms, but expected to complete in " + UNTRHOTTLED_REQUEST_TIME_MS
        + "ms",
        finish - start, lessThan((long) UNTRHOTTLED_REQUEST_TIME_MS));
  ...
  public void testChangeThrottling() throws IOException {
    ...
    assertThat("Expected second read to take approximately half as long as first throttled read. First read took " + (
            firstFinish - firstStart) + "ms" + " but second read took " + (secondFinish - secondStart) + "ms",
        (double) (firstFinish - firstStart) / 2,
        both(greaterThan((secondFinish - secondStart) * (1 - ALLOWABLE_VARIATION)))
  ...
  public void testDisableThrottling() throws IOException {
    ...
    assertThat("Expected second read to complete within " + UNTRHOTTLED_REQUEST_TIME_MS
               + "ms, without throttling. First read took "
